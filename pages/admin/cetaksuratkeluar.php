...
if (isset($_POST['dari']) && isset($_POST['ke'])) {
  ...
  $data = mysqli_query($koneksi, "SELECT s.*, p.nama_perihal, b.nama_bidang FROM tbl_suratkeluar s, tbl_perihal p, tbl_bidang b WHERE s.id_perihal = p.id AND s.id_bidang = b.id AND s.status = 'disetujui' AND s.tanggal_keluar BETWEEN '" . $_POST['dari'] . "' and '" . $_POST['ke'] . "'");
} else if (empty($_POST['dari']) && empty($_POST['ke'])) {
  //jika tidak ada waktupanen dari dan waktupanen ke maka tampilkan seluruh data
  $data = mysqli_query($koneksi, "SELECT s.*, p.nama_perihal, b.nama_bidang FROM tbl_suratkeluar s, tbl_perihal p, tbl_bidang b WHERE s.id_perihal = p.id AND s.id_bidang = b.id AND s.status = 'disetujui'");
} else {
  $data = mysqli_query($koneksi, "SELECT s.*, p.nama_perihal, b.nama_bidang FROM tbl_suratkeluar s, tbl_perihal p, tbl_bidang b WHERE s.id_perihal = p.id AND s.id_bidang = b.id AND s.status = 'disetujui'");
}
?>
