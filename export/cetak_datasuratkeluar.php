...
<table>
        ...
        <tr>
            <div class="table-responsive mt-3">
                <table border="1" width="90%" align="center" cellpadding="4">
                    <thead>
                        <br>
                        <h2 align="center">Laporan Data Surat Keluar</h2>
                        <th>No</th>
                        <th>Tujuan Surat</th>
                        <th>Bidang</th>
                        <th>No Surat</th>
    ...
    if (isset($_GET['id'])) {
        $selectSql = "SELECT s.*, p.nama_perihal, b.nama_bidang FROM tbl_suratkeluar s, tbl_perihal p, tbl_bidang b WHERE s.id_perihal = p.id AND s.id_bidang = b.id AND s.id_surat = '$_GET[id]'";
    } else {
        $selectSql = "SELECT s.*, p.nama_perihal, b.nama_bidang FROM tbl_suratkeluar s, tbl_perihal p, tbl_bidang b WHERE s.id_perihal = p.id AND s.id_bidang = b.id AND s.status = 'disetujui'";
    }
    ...
    while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
        $pengirim = $row['tujuan_surat'];
        $bidang = $row['nama_bidang'];
        $nosurat = $row['nomor_surat'];
        $tglmasuk = $row['tanggal_keluar'];
    ...
    ?>
        <tr>
            ...
            <td align="center"><?php echo $row['tujuan_surat'] ?></td>
